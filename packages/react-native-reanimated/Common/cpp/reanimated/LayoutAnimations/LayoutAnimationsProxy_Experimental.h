...
struct TransactionMeta {
  ...
  std::vector<std::shared_ptr<LightNode>> exiting;
...
struct LayoutAnimationsProxy_Experimental : public LayoutAnimationsProxyCommon {
  ...
  mutable std::optional<PendingSourceRemoval> pendingSourceRemoval_;
  ...
  void handleProgressTransition(
      ...
      bool pendingResolvedThisPull) const;
  void resolveTransitionLifecycle(
      TransactionMeta &transaction,
      const ShadowViewMutationList &mutations,
      const PropsParserContext &propsParserContext) const;
  bool settlePendingSourceRemoval(TransactionMeta &transaction) const;
  ...
  void hideTransitioningViews(
      BeforeOrAfter index,
      const CollectedTransitions &transitions,
