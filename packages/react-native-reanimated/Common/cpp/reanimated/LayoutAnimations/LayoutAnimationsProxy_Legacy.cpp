        ...
        if (movedViews.contains(tag)) {
          if (const auto currentView = reparentLayoutAnimation(tag, movedViews.at(tag))) {
            filteredMutations.push_back(
                ShadowViewMutation::InsertMutation(mutationParent, *currentView, mutation.index));
          } else if (oldShadowViewsForReparentings.contains(tag)) {
            filteredMutations.push_back(
                ShadowViewMutation::InsertMutation(mutationParent, oldShadowViewsForReparentings[tag], mutation.index));
          } else {
            filteredMutations.push_back(mutation);
          }
          continue;
